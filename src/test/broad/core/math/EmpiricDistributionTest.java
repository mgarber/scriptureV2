...
public class EmpiricDistributionTest extends TestCase {
	...
	public void testGetNumberOfValuesLargerThan() throws IOException {
		...
		assertEquals("P(x < 0.5) = 0 since there is only one datapoint" ,0.0, ed.getCumulativeProbability(0.5));
		assertEquals("P(x < 2) = 1 since there is only one datapoint that is less than 2", 1.0, ed.getCumulativeProbability(2));
		...
		assertEquals("P(x < 2) = 1/2 since there is only one datapoint" ,0.5, ed.getCumulativeProbability(2));
		assertEquals("P(x < 9) = 1 since both datapoints are less 9", 1.0, ed.getCumulativeProbability(9));
		assertEquals("P(x < 8.87) = 1/2", 0.5, ed.getCumulativeProbability(8.87));
		...
		assertEquals("P(x < 2) = 1/2 since there is only one datapoint" ,1.0/3.0, ed.getCumulativeProbability(2));
		assertEquals("P(x < 9) = 1 since both datapoints are less 9", 1.0, ed.getCumulativeProbability(9));
		assertEquals("P(x < 8.93) = 22/3", 2.0/3.0, ed.getCumulativeProbability(8.93));
